...
<TwoColumns>
  <Column className="flex flex-col justify-center items-center gap-4">
    ...
    <div className="flex flex-row gap-2 mt-8 w-full justify-center">
      <Link onClick={(e) => e.stopPropagation()} href="/demo/cache/1">
        Cache demo
      </Link>
    </div>
  </Column>
...
When it comes to data caching you can also use the cache method to cache data, but this is not connected to the page cache. This is a way to cache data that is not connected to a page, and you can use this to cache data that is used in multiple places. 

When it comes to page caching you can also use the unstable_revalidate method to set how often you want to revalidate the page. This is a good way to make sure that you have fresh data on the page, but be aware that this will also make the page slower since it needs to revalidate the data.
